...
struct lzxd_stream {
  ...
  long int   offset;                 /* number of bytes actually output         */
  long int   length;                 /* overall decompressed length of stream   */
 ...
 * @return an error code, or MSPACK_ERR_OK if successful
 */
extern int lzxd_decompress(struct lzxd_stream *lzx, long int out_bytes);
