...
{
  result->data[0] = BE64(result->data[0]);
  result->data[1] = BE64(result->data[1]);
  result->data[2] = BE64(result->data[2]);
  result->data[3] = BE64(result->data[3]);

  for (uint32_t i = 0; i < input_qwords; i++)
  {
    uint64_t data = BE64(*input);
    input++;
